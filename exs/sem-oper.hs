...
subs (ExpSeq e1 e2) y l = ExpSeq (subs e1 y l) (subs e2 y l)
  -- 064
                                                                          -- 065
...
step (ExpSeq e1 e2) m = (comma e1 e2, m) 
  
--step (ExpSeq (ExpK n) (ExpK n')) m =  ( (comma (ExpK n) (ExpK n')),m) 
  
--step (ExpSeq v e) m | isValue v= (ExpSeq v e,m')  
--  where (e', m') = step e m 
  
--step (ExpSeq e1 e2) m =	(ExpSeq e1' e2,m')
--	where (e1', m') = step e1 m 
 -- 101
...
t3 = letin "f" (ExpLambda "x" (ExpArith (ExpVar "x") OpAdd (ExpK 10)))    -- 121
       (letin "x" (ExpK 15)                                               -- 122
          (ExpSeq(ExpAg "x" (ExpApp (ExpVar "f") (ExpVar "x"))) (ExpVar "x")))

t4 = 	ExpSeq (ExpK 10)(ExpK 20)	  -- 130
main = print(eval t3 [])                                                  -- 131